...
struct ContentView: View {
    ...
    var body: some View {
        ...
        .alert("location_saved".localized, isPresented: $showingSaveAlert) {
            ...
            Text("parking_saved_successfully".localized)
        }
    ...
    private var headerSection: some View {
        VStack(spacing: 12) {
            ...
            HStack(spacing: 6) {
                Circle()
                    .fill(locationManager.userLocation != nil ? Color.green : Color.orange)
                    .frame(width: 8, height: 8)
                
                Text(locationManager.userLocation != nil ? "ready_to_save".localized : "getting_location".localized)
                    .font(.caption)
    ...
    private var saveButton: some View {
        Button(action: saveParkingWithAnimation) {
            HStack(spacing: 12) {
                if isSaving {
                    ProgressView()
                        .scaleEffect(0.9)
                ...
                Text(isSaving ? "saving".localized : "save_parking".localized)
                    .font(.headline)
                    .fontWeight(.semibold)
            }
            .foregroundColor(.white)
            ...
            .background(
                Group {
                    if locationManager.userLocation != nil && !isSaving {
                        LinearGradient(
        ...
        .disabled(locationManager.userLocation == nil || isSaving)
        .buttonStyle(PlainButtonStyle())
    }
